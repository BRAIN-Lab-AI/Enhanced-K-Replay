import sys
sys.path.append('/content/drive/My Drive/DL Project/KnowCap-master/models/OFA')
import torch
...
class OFA(nn.Module):
    ...
    def __init__(self, config, distill_model=False, use_patch_self_attn=False):
        ...
        if distill_model:
            ...
            self.ofa_model = OFAModel.from_pretrained(self.config.ofa_ckpts, use_cache=False, use_patch_self_attn=self.use_patch_self_attn,).to(device)
        #self.ofa_encoder = self.ofa_model.encoder
        self.prompt = " what does the image describe?"
        #self.prompt = " what does the image describe? and what all landmarks, famous brands, special foods, movie characters in this image?"
    ...
    def greedy_search(self, patch_img, mode='max'):
        ...
        patch_img: [batch_size, *img_patch_size]
        """
        # Greedy search: the returned tokens should include both the start and end tokens so they can be used as pseudo-captions.
        fixed_len = self.config.fixed_len
        gen_num = self.config.beam_num if mode == 'prob' else 1
        batch_size = patch_img.shape[0]*gen_num
        # The BOS (beginning-of-sequence) token in the OFA model is 0
